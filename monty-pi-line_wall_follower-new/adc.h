...
class AnalogueConverter {
 public:
  ...
  void converter_init() {
    ...
    analogin_init(&m_halObject[0], p26);  // Start/stop
    analogin_init(&m_halObject[1], p27);  // Right line
    analogin_init(&m_halObject[2], p28);  // Left line
    analogin_init(&m_halObject[3], p29);  // Radius
...
*/
  ...
  int get_raw(const int i) const {
    int diff;
    ATOMIC {
#if SENSORS_INVERTED_POLARITY
      diff = max(1, m_adc_dark[i] - m_adc_lit[i]);
#else
