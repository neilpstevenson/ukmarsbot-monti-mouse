 ...
 public:
  ...
  void update() {
    ...
    if (g_steering_mode == STEER_NORMAL) {
      ...
      error = -2 * right_error;
    } else if (g_steering_mode == STEER_LINE_FOLLOW) {
    ...
    // Indicate which walls seen
    #ifdef DEBUG_RAW_SENSOR_TICK
    SerialPort.print(lss.raw); SerialPort.print(", ");
    SerialPort.print(lfs.raw); SerialPort.print(", ");
    SerialPort.print(rfs.raw); SerialPort.print(", ");
    SerialPort.println(rss.raw);
    #endif
    digitalWrite (LED_LEFT, see_left_wall);   // Left/Green LED
    digitalWrite (LED_RIGHT, see_right_wall);  // Right/Red LED
